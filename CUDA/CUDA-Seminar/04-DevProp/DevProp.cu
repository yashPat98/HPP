...
void PrintCUDADeviceProperties(void)
{
    ...
    cudaError_t ret_cuda_rt = cudaSuccess;
    int dev_count;
    ...
    {
        printf("There Is No CUDA Support Device On This System. Exiting Now ...\n");
        return ;
    }
    else
    {
        printf("Total Number of CUDA Supporting Device/Devices On This System : %d\n", dev_count);
        for(int i = 0; i < dev_count; i++)
        {
            ...
            if(ret_cuda_rt != cudaSuccess)
            {
                printf("CUDA Runtime API Error - cudaGetDeviceProperties() Failed Due To %s. Exiting Now ...\n", cudaGetErrorString(ret_cuda_rt));
                return ;
            }
