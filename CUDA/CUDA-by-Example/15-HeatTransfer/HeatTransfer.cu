...
int main(void)
{
    ...
    //bind textures
    err = cudaBindTexture(NULL, texConstSrc, data.device_constSrc, bitmap.image_size());
    if(err != cudaSuccess)
    {
        printf("GPU Error - cudaBindTexture() failed : %s.\n", cudaGetErrorString(err));
        cleanup(&data);
        exit(EXIT_FAILURE);
    }

    err = cudaBindTexture(NULL, texIn, data.device_inSrc, bitmap.image_size());
    if(err != cudaSuccess)
    {
        printf("GPU Error - cudaBindTexture() failed : %s.\n", cudaGetErrorString(err));
        cleanup(&data);
    ...
    err = cudaBindTexture(NULL, texOut, data.device_outSrc, bitmap.image_size());
    if(err != cudaSuccess)
    {
        printf("GPU Error - cudaBindTexture() failed : %s.\n", cudaGetErrorString(err));
        cleanup(&data);
